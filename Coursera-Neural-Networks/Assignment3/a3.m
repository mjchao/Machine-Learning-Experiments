...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  scale = 1/size(data.inputs,2);
  for i = 1:size(data.inputs, 2)
      inputs = data.inputs(:,i);
      targets = data.targets(:,i);
      
      z = model.input_to_hid * inputs;
      h = logistic(z);
      zeta = model.hid_to_class * h;
      g = exp(zeta - repmat(log_sum_exp_over_rows(zeta), [size(zeta, 1), 1]));
      %E = -mean(sum(log(g) .* targets, 1));

      % computation for hid_to_class
      partial_E_partial_g = g - targets;
      partial_g_partial_zeta = g .* (1 - g);
      partial_g_partial_zeta = 1;
      ret.hid_to_class = ret.hid_to_class + scale * ((partial_E_partial_g .* partial_g_partial_zeta) * h' + wd_coefficient .* model.hid_to_class);

      % computation for input_to_hid
      partial_E_partial_h = sum(repmat(partial_E_partial_g .* partial_g_partial_zeta, [1, size(model.hid_to_class, 2)]) .* model.hid_to_class, 1);
      partial_E_partial_z = partial_E_partial_h' .* h .* (1-h);
      partial_E_partial_w = partial_E_partial_z * inputs';
      ret.input_to_hid = ret.input_to_hid + scale * (partial_E_partial_w + wd_coefficient .* model.input_to_hid);
  end
end
